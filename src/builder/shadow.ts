...
export function boxShadow(
  ...
  for (let i = style.boxShadow.length - 1; i >= 0; i--) {
    ...
    if (shadowStyle.inset) {
      ...
      const left = Math.min(-grow, 0)
      const right = Math.max(grow + width, width)
      const top = Math.min(-grow, 0)
      const bottom = Math.max(grow + height, height)
