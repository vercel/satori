...
const buildDropShadow = (
	...
	for (let i = 0; i < shadowCount; i++) {
		...
		const grow =
			((style.shadowRadius[i] || 0) * (style.shadowRadius[i] || 0)) / 4;
		...
		if (transparentText) {
			...
			effects +=
				buildXMLString('feGaussianBlur', {
					...
					stdDeviation: (style.shadowRadius[i] || 0) / 2
				}) +
...
const boxShadow = (
	...
	for (let i = style.boxShadow.length - 1; i >= 0; i--) {
		...
		// Expand the area for the filter to prevent it from cutting off.
		const grow =
			((shadowStyle.blurRadius || 0) * (shadowStyle.blurRadius || 0)) /
				4 +
			...
			buildXMLString(
				...
				buildXMLString(
					...
					buildXMLString('feGaussianBlur', {
						...
						// > https://www.w3.org/TR/css-backgrounds-3/#shadow-blur
						result: 'b',
						stdDeviation: (shadowStyle.blurRadius || 0) / 2
					}) +
						buildXMLString('feFlood', {
