...
const UNICODE_SVG =
  '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="50"><text>İstanbul 東京</text></svg>'
...
beforeEach(() => {
  ...
  ;(globalThis as any).fetch = async (url) => {
    ...
    if (url.endsWith('unicode.svg')) {
      return {
        headers: { get: () => 'image/svg+xml' },
        text: async () => UNICODE_SVG,
      }
    }
...
describe('Image', () => {
  it.each([
    `data:image/svg+xml,${encodeURIComponent(UNICODE_SVG)}`,
    'https://example.com/unicode.svg',
  ])('should preserve UTF-8 content in SVG images from %s', async (source) => {
    const [resolved, width, height] = await resolveImageData(source)
    expect([width, height]).toEqual([100, 50])
    expect(Buffer.from(resolved.split(',')[1], 'base64').toString('utf8')).toBe(
      UNICODE_SVG
    )
  })
